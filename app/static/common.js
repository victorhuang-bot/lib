async function api(url,opt={}){
  ...
  for(let attempt=0;attempt<(safeRetry?2:1);attempt++){
    ...
    if(r.status===403)throw new Error('目前登入角色沒有此操作權限；若同時開啟管理者與總館頁籤，請重新整理此頁確認角色。');
...
function statusText(s){return ({WAITING_SECRETARY:'待總館輸入',WAITING_DRIVER:'待司機輸入',WAITING_BRANCH:'待分館簽收',LATE_BRANCH_PENDING:'運送完成・待分館補簽',WAITING_DRIVER_CONFIRM:'分館已簽・待司機確認',WAITING_BRANCH_CORRECTION:'待分館更正',WAITING_DRIVER_RECONFIRM:'已更正・待司機確認',STOP_COMPLETED:'本站完成',VOID:'已作廢'})[s]||s}
